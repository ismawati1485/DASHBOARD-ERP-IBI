...
function InventoryPage() {
  const totalSales = salesPerCategory.reduce((a, b) => a + b.totalSales, 0);
  const pieData = salesPerCategory.map((c) => ({
    ...c,
    percentage: +((c.totalSales / totalSales) * 100).toFixed(1),
  }));
            ...
            <TableBody>
              {stockList.map((s, idx) => (
                <TableRow key={s.itemCode}>
                  ...
                  <TableCell className="text-muted-foreground">{fmtDate(s.tanggalTerakhirMasuk)}</TableCell>
